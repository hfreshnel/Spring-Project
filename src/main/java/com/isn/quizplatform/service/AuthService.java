...
public class AuthService {
	...
	public ResponseEntity<ApiResponse<Personne>> register(Personne personne) {
		String emailRegex = "^[\\w.-]+@[\\w.-]+\\.[a-zA-Z]{2,}$";
			if(!personne.getMail().matches(emailRegex)){ //check from of email
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body(new ApiResponse<>(null, 404, "auth.email_from_wrong"));
			}
			if(PR.findByMail(personne.getMail()).isPresent()){ // check the email is? exits
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body(new ApiResponse<>(null, 404, "auth.user_already_exists"));
			}
			if(personne.getMdp().length() < 6){ // check the length of mdp
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body(new ApiResponse<>(null, 404, "auth.mdp_form_wrong"));
			}else {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.OK)
					.body(new ApiResponse<>(personne, 200, null)); // Succès, utilisateur créé
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(new ApiResponse<>(null, 500, "auth.registration_failed")); // Erreur interne
		}
	}

	//User login
	public ResponseEntity<ApiResponse<AuthResponse>> login(String email, String password) {
		try {
			...
			return ResponseEntity.status(HttpStatus.OK)
					.body(new ApiResponse<>(auth, 200, null));
		} catch (RuntimeException e){
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new ApiResponse<>(null, 404, e.getMessage()));
		} catch (Exception e){
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(new ApiResponse<>(null, 500, "auth.login_failed"));
		}
	}
